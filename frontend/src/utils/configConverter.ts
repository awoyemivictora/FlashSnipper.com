...
export interface BackendLaunchConfig {
  ...
  priority?: number;
  // ✅ ADD THESE NEW FIELDS for dynamic bot amounts
  bot_variability?: number;          // 0-1 (0-100% variability)
  bot_distribution?: string;         // 'normal', 'uniform', 'log_normal', 'random'
  min_buy_amount?: number;           // Optional minimum override
  max_buy_amount?: number;           // Optional maximum override
  
  // ✅ Optional: Add more dynamic settings if needed
  stagger_buys?: boolean;            // Whether to stagger buys over time
  buy_delay_ms?: number;             
}
...
export const convertToBackendConfig = (config: LaunchConfig): BackendLaunchConfig => {
  const backendConfig: BackendLaunchConfig = {
    ...
    bot_variability: config.botVariability || 0.3,
    bot_distribution: config.botDistribution || 'normal',
    min_buy_amount: config.minBuyAmount,
    max_buy_amount: config.maxBuyAmount,
    stagger_buys: config.staggerBuys,
    buy_delay_ms: config.buyDelayMs,
  };
