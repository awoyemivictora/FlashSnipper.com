...
const RPC_ENDPOINT = 'https://rpc.shyft.to?api_key=0C53vJyghxriRpQX';
const connection = new Connection(RPC_ENDPOINT, 'confirmed');

export async function getTokenPrice(mintAddress, authToken) {
  if (!mintAddress) throw new Error('Mint address is required');
...
export async function getWalletTokenBalances(walletAddress, authToken) {
  try {
    const response = await apiService.request(`/wallet/balances?wallet_address=${encodeURIComponent(walletAddress)}`, {
      method: 'GET',
      headers: { Authorization: `Bearer ${authToken}` },
    });
    logger.info(`✅ Fetched token balances for ${walletAddress}`);
    return response.balances;
  } catch (error) {
    logger.error(`❌ Failed to fetch token balances for ${walletAddress}: ${error.message}`);
    throw new Error(`Could not fetch token balances: ${error.message}`);
  }
}
