...
//   const handleWalletClick = (walletName: string, readyState: WalletReadyState) => {
//     // Just select the wallet - let the wallet adapter handle everything
//     // The wallet adapter knows how to handle mobile/desktop properly
//     select(walletName as any);
//     handleClose();
//   };
...
//               key={wallet.adapter.name as string}
//               onClick={() => handleWalletClick(wallet.adapter.name as string, wallet.readyState)}
//               className="flex items-center gap-3 p-3 w-full rounded-lg transition-all duration-200 hover:bg-gray-800/50 bg-gray-800/30"
//             >
//               {wallet.adapter.icon && (
//                 <img 
//                   src={wallet.adapter.icon} 
...
//   // Connected State - SIMPLIFIED VERSION
//   if (connected && publicKey) {
//     return (
//       <div className="relative">
//         <button
//           onClick={() => setShowAccountMenu(!showAccountMenu)}
//           className={`
//             group flex items-center gap-2 bg-gradient-to-r from-gray-900 to-gray-800 
//             hover:from-gray-800 hover:to-gray-700 border border-gray-700/50 
//             rounded-xl px-4 py-2.5 transition-all duration-200 hover:border-gray-600/50
//             hover:shadow-lg ${className}
//           `}
...
//             />
//           ) : (
//             <div className="w-5 h-5 bg-gradient-to-br from-cyan-500 to-blue-500 rounded flex items-center justify-center">
//               <Wallet className="w-3 h-3 text-white" />
//             </div>
//           )}
...
//         {/* SIMPLIFIED Account Dropdown Menu */}
//         {showAccountMenu && (
//           <div className="absolute top-full mt-2 right-0 w-48 bg-gray-900/95 border border-gray-700/50 rounded-xl shadow-2xl z-50 overflow-hidden backdrop-blur-sm animate-in slide-in-from-top-5 duration-200">
//             {/* Header */}
//             <div className="p-4 border-b border-gray-800/50">
//               <div className="flex items-center gap-2">
//                 {walletIcon && (
...
//                 )}
//                 <div>
//                   <div className="text-white font-medium text-sm">{walletName}</div>
...
//                 onClick={handleDisconnect}
//                 disabled={disconnecting}
//                 className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-red-600/20 hover:bg-red-600/30 text-red-400 hover:text-red-300 rounded-lg transition-colors text-sm"
//               >
//                 {disconnecting ? (
//                   <>
//                     <Loader2 className="w-3 h-3 animate-spin" />
...
const MOBILE_WALLET_CONFIGS = {
  phantom: {
    ...
    readyStateOnMobile: WalletReadyState.Loadable,
  },
  solflare: {
    name: 'Solflare',
    scheme: 'solflare://',
    appStoreUrl: 'https://solflare.com/download',
    deepLinkUrl: 'https://solflare.com/ul/browse/{encodedUrl}',
    readyStateOnMobile: WalletReadyState.Loadable,
  },
  trust: {
    name: 'Trust Wallet',
    scheme: 'trust://',
    appStoreUrl: 'https://trustwallet.com/solana-wallet',
    deepLinkUrl: 'https://link.trustwallet.com/open_url?url={encodedUrl}',
    readyStateOnMobile: WalletReadyState.Loadable,
  },
};
...
const isMobileWalletInstalled = (walletName: string): boolean => {
  ...
  // For mobile, we assume wallets are available via deep links
  // This is the industry standard approach
  return ['phantom', 'solflare', 'trust'].some(name => 
    walletLower.includes(name)
  );
};
...
export const WalletProvider: FC<{ 
  ...
  // Initialize wallets with PROPER mobile handling
  const wallets = useMemo(() => {
    const adapters: BaseWalletAdapter[] = [];
    
    // IMPORTANT: Initialize wallets in a specific order
    // 1. Phantom (mobile-friendly)
    const phantomAdapter = new PhantomWalletAdapter();
    adapters.push(phantomAdapter);
    
    // 2. Solflare (mobile-friendly)
    const solflareAdapter = new SolflareWalletAdapter({ network: currentNetwork });
    adapters.push(solflareAdapter);
    
    // 3. Trust Wallet (mobile-friendly via WalletConnect)
    const trustAdapter = new TrustWalletAdapter();
    adapters.push(trustAdapter);
    
    console.log(`Initialized ${adapters.length} wallet adapters`);
    return adapters;
  }, [currentNetwork]);
  ...
  const endpoint = useMemo(() => {
    ...
    return rpcUrls[currentNetwork] || rpcUrls[WalletAdapterNetwork.Mainnet];
  }, [currentNetwork, customEndpoint]);
...
const WalletContextInner: FC<{
  ...
  // Enhanced connect function for mobile
  const connect = useCallback(async (walletName?: string) => {
    try {
      clearError();
      ...
      if (walletName) {
        ...
        if (targetWallet) {
          // On mobile, handle wallet-specific deep links
          if (isMobile && targetWallet.readyState === WalletReadyState.NotDetected) {
            const walletLower = targetWallet.adapter.name.toLowerCase();
            
            // Check if it's a mobile wallet that supports deep links
            if (walletLower.includes('phantom') || walletLower.includes('solflare') || walletLower.includes('trust')) {
              // For mobile wallets, we'll let the modal handle the deep link
              // Just select it and let the adapter handle it
              select(targetWallet.adapter.name);
              return;
            }
          }
          
          // Normal selection for other cases
          select(targetWallet.adapter.name);
        } else {
          setError(`Wallet "${walletName}" not found. Try using the wallet selector.`);
        }
      } else {
        // Open modal if no wallet specified
        const { setVisible } = useWalletModal();
        setVisible(true);
      }
    } catch (error: any) {
  ...
  }, [availableWallets, select, setError, clearError, isMobile]);
  ...
  const signAllTransactions = useCallback(async (transactions: Transaction[]): Promise<Transaction[]> => {
    try {
      ...
      return await solanaSignAllTransactions(transactions);
    } catch (error: any) {
      console.error('Failed to sign transactions:', error);
      setError(error.message || 'Failed to sign transactions. Please try again.');
      throw error;
  ...
  const getWalletCategory = (
    ...
    if (isMobileDevice && (name.includes('trust') || name.includes('phantom'))) {
      return 'mobile';
    }
    ...
    availableWallets.forEach(wallet => {
      ...
      if (!uniqueWallets.has(walletName)) {
        uniqueWallets.set(walletName, {
          ...
          category: getWalletCategory(walletName, isMobile)
        });
      }
    });
          ...
          {filteredWallets.length === 0 ? (
            ...
            filteredWallets.map((wallet) => {
              ...
              return (
                <button
                  key={wallet.name}
                  onClick={() => handleWalletClick(wallet.name, wallet.readyState)}
                  disabled={wallet.readyState === WalletReadyState.NotDetected}
                  title={wallet.readyState === WalletReadyState.NotDetected ? 
                    `${wallet.name} is not installed. Click for installation instructions.` : 
                    `Connect with ${wallet.name}`}
