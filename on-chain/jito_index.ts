...
const TOKEN_METADATA_PROGRAM_ID = new PublicKey("metaqbxxUerdq28cj1RbAqKECAQaqeVTn5scarwxpg8");
...
async function info(pair) {
  ...
  const accInfo = await connection.getAccountInfo(metadataPDA);
    ...
    const { name: tokenName, symbol: tokenSym } = decodeMetadata(accInfo.data);
  ...
  // const qvault = info.quoteVault;
   const owner = info.owner;    // comes from Raydium_LiquidityPoolv4
