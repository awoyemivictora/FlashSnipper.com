...
export async function createCompleteLaunchBundle(
  ...
  try {
    console.log(`🚀 Creating pump.fun launch...`);
...
//     units: 320000 // From DIEGO transaction
//   });
  
//   const computeUnitPriceIx = ComputeBudgetProgram.setComputeUnitPrice({
//     microLamports: 350000 // 0.35 lamports per CU from DIEGO
//   });
...
//   // ============================================
//   // 5. CREATOR BUY (with small amount and 0 slippage)
//   // ============================================
//   const lamportsIn = BigInt(Math.floor(creatorBuyAmount * LAMPORTS_PER_SOL));
  
//   // Use regular buy (not buyExactSolIn) for creator with 0 slippage
//   // Check if you have a regular buy instruction builder, or use buyExactSolIn with minTokenOut = 0
//   const buyIx = PumpFunInstructionBuilder.buildBuyExactSolIn(
//     userKeypair.publicKey,
//     mint,
//     creatorAta,
//     userKeypair.publicKey,
//     lamportsIn,
//     BigInt(0) // 0 slippage for creator
//   );
...
async function buildSimpleTokenCreationWithBuyTx(
  ...
  // ============================================
  // 2. CREATE_V2 (USE ACTUAL METADATA!)
  // ============================================
  const createIx = PumpFunInstructionBuilder.buildCreateV2(
    userKeypair,
    mintKeypair,
    metadata.name || 'Token',
    metadata.symbol || 'TOK',
    metadata.uri || "https://arweave.net/example", // Use actual URI
    false,
    userKeypair.publicKey
  );
